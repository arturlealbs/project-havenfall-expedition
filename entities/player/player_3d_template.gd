...
@export var wall_slide_gravity := -25.0
@export var wall_offset := 0.15
...
@onready var _ground_dust_particles: GPUParticles3D = %GroundDustParticles
@onready var _wall_dust_particles = %WallDustParticles
@onready var eye_ray_cast = $SophiaSkin/HandRayCast
@onready var head_ray_cast = $SophiaSkin/HeadRayCast
...
func _physics_process(delta: float) -> void:
	...
	# Calculate turn for tilt animation
	var current_yaw := _camera_pivot.rotation.y
	var yaw_delta := wrapf(current_yaw - _last_yaw, -PI, PI)
	_last_yaw = current_yaw
	var turn_speed := yaw_delta / delta
	#turn_speed += raw_input.x
	_smoothed_turn = lerpf(_smoothed_turn, turn_speed, 6.0 * delta)
	var target_tilt := -_smoothed_turn * 0.25
	target_tilt = clamp(target_tilt, -1.0, 1.0)
	...
	var is_wall_sliding :bool = not is_on_floor() and eye_ray_cast.is_colliding() and y_velocity <= 0.0
	var is_on_ledge :bool = not head_ray_cast.is_colliding() and eye_ray_cast.is_colliding() and y_velocity <= 0.0
	velocity.y = 0.0
	var input_strength = raw_input.length()  # já vem do Input.get_vector
	var target_speed = lerp(move_speed, running_speed, Input.is_action_pressed("run"))
	velocity = velocity.move_toward(move_direction * target_speed * input_strength, acceleration * delta)
	...
	var is_just_jumping := Input.is_action_just_pressed("ui_jump") and (is_on_floor() or is_on_ledge)
	if is_just_jumping:
		...
		if ground_speed > 0.0:
			...
			var blend = clamp(
				inverse_lerp(move_speed, running_speed, current_speed),0.0,1.15)
			_skin._set_run_speed(blend)
			target_tilt += raw_input.x 
			_skin._set_run_tilt(target_tilt)
		else:
			_skin.idle()

	_ground_dust_particles.emitting = is_on_floor() && ground_speed > 0.0
	_wall_dust_particles.emitting = eye_ray_cast.is_colliding() && velocity.y < 0.0
